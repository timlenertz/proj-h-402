...
private:
	...
	tree_structure_node* children_[Splitter::number_of_node_children]; ///< Child nodes, either all set of all null.
	point_set point_sets_[Levels]; ///< Point sets for different downsampling levels.
	 ...
	 * Adds point into internal buffer of a leaf.
	 * @see add_root_node_points
...
public:
	...
	point_iterator points_begin(std::ptrdiff_t lvl = 0) const { assert(lvl >= 0 && lvl < Levels); return point_sets_[lvl].points_iterator; } ///< Get iterator to start of point set.
	point_iterator points_end(std::ptrdiff_t lvl = 0) const { assert(lvl >= 0 && lvl < Levels); return point_sets_[lvl].points_iterator + point_sets_[lvl].number_of_points; } ///< Get iterator to end of points set.
	...
	std::size_t number_of_points(std::ptrdiff_t lvl = 0) const { assert(lvl >= 0 && lvl < Levels); return point_sets_[lvl].number_of_points; } ///< Get number of points in node for given downsampling level.
