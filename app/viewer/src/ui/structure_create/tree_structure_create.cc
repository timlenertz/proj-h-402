...
dypc_loader tree_structure_create::create_memory_loader(dypc_model mod) const {
	...
	if(i >= 0 && i < 3) {
		return dypc_create_tree_structure_loader(mod, structure_type_, selected_levels_(), leaf_cap, mmfac, dmode, dmax, ltypes[i]);
	} else {
		return nullptr;
	}
}
...
void tree_structure_create::write_structure_file(dypc_model mod, const std::string& filename) const {
	...
	dypc_write_tree_structure_to_file(filename.c_str(), mod, structure_type_, selected_levels_(), leaf_cap, mmfac, dmode, dmax);
}
